...
async function downloadMedia(
  ...
  try {
    ...
    await FileModule.writeFile("documents", saveName, base64, "base64");

    const { DocumentDirectory } = FileModule.getConstants();
    return `file://${DocumentDirectory}/${saveName}`;
  } catch {
    return null;
  }
}
